...
bool hr_process_sample(int ecg_value) {
    ...
    // Use filtered_value for peak detection
    if (filtered_value > HR_DETECTION_THRESHOLD && 
...
bool hr_check_leads_connected(void) {
    // get_electrode_status() returns ELECTRODE_NORMAL (0) if connected, ELECTRODE_OFF (1) if disconnected.
    // So, return true if status is ELECTRODE_NORMAL.
    return (get_electrode_status() == ELECTRODE_NORMAL);
}
